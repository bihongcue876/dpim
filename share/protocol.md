...
用于 WebUI 中确保用户操作基于最新数据状态，防止陈旧页面覆盖后续变更。

**机制：**

1. 后端在所有数据操作（事件/节点增删改）完成后更新内部状态版本号
2. `GET /state-hash` 返回当前状态的 MD5 摘要（基于最大 event_id + 节点总数 + 最近修改时间戳）
3. WebUI 加载时获取 `state_hash`，交互控件初始为锁定态
4. 用户点击"更新"按钮，重新请求 `state-hash`
5. 若哈希一致 → 解锁交互控件
6. 若哈希不一致 → 提示"数据已变更"，保持锁定
7. 刷新页面重新加载，初始仍为锁定态

**规则：**
- 哈希仅用于前端一致性校验，不影响后端写入路径
- 后端写入操作不依赖哈希检查（保证后端接口无状态）
- 哈希冲突概率极低（MD5 + 多因子），实际可忽略
