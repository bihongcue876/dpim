...
class TestSearchSemantic:
    """语义检索三路融合：向量路召回 / 失败回退 / 未启用不调用"""

    @pytest.fixture
    def embed_enabled(self, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_vector_recall_only_result(self, db, embed_enabled, monkeypatch):
        """对照：关键词不命中时，仅向量路召回语义相似事件"""
        from core.embeddings import EmbeddingStore
        from core.models import SearchRequest

        async def fake_embed(texts, role="cr"):
            # search 只传 [query]；此处返回固定查询向量
            return [[1.0, 0.0, 0.0] for _ in texts]
        monkeypatch.setattr("core.llm.gateway.embed", fake_embed)
        es = EventStore(db)
        eid, _ = await es.insert_event("语义相似的内容主题", "interaction")
        await EmbeddingStore(db).upsert_event(eid, [1.0, 0.0, 0.0], "test-model")
        resp = await search(
            SearchRequest(query="zzz-never-keyword", limit=5),
            es, GraphStore(db), degraded=False,
        )
        ids = [r.node_id for r in resp.results]
        assert eid in ids

    @pytest.mark.asyncio
    async def test_vector_failure_falls_back(self, db, embed_enabled, monkeypatch):
        """对照：嵌入调用失败 → 静默回退，行为与两路一致（不抛异常）"""
        from core.embeddings import EmbeddingStore
        from core.models import SearchRequest
        ...
        await EmbeddingStore(db).upsert_event("semantic_event", [1.0, 0.0, 0.0], "m")
        resp = await search(
            SearchRequest(query="zzz-never-keyword", limit=5),
            EventStore(db), GraphStore(db), degraded=False,
        )
        assert resp.results == []  # 关键词不命中 + 向量路失败 → 无结果，不崩溃

    @pytest.mark.asyncio
    async def test_disabled_does_not_call_embed(self, db, monkeypatch):
        """对照：未配置 embedding_model → 不调用嵌入接口"""
        from core.config import settings
        from core.models import SearchRequest
    ...
    @pytest.mark.asyncio
    async def test_degraded_skips_vector(self, db, embed_enabled, monkeypatch):
        """对照：降级模式（AI 不可用）不调用嵌入接口"""
        from core.models import SearchRequest
        calls: list[str] = []
        async def fake_embed(texts, role="cr"):
            calls.append("x")
        ...
        await search(
            SearchRequest(query="Python", limit=5),
            EventStore(db), GraphStore(db), degraded=True,
        )
        assert calls == []
