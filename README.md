...
- 混合检索：FTS5 关键词召回 + 图扩散 + 可选语义向量召回（embedding）三路 RRF 融合排序（无 Agent 默认仅 FTS5；嵌入不可用自动回退两路）
- **语义检索（可选增强）**：配置 EMBEDDING_MODEL 后启用，事件/节点自动生成嵌入（SQLite 向量表 + 纯标准库余弦），检索时三路融合；嵌入失败静默回退不影响事件状态。嵌入服务可独立配置（EMBEDDING_BASE_URL / EMBEDDING_API_KEY，空 = 跟随活动提供商），支持与主 LLM 不同厂商（如 DeepSeek 生成 + SiliconFlow 嵌入）
...
| 标签页 | 功能 |
|--------|------|
| **配置** | 系统参数可视化编辑（存储/模型与提供商/Agent 管线/检索/系统/前端 6 板块；语义检索配置并入「模型与提供商」：嵌入模型/维度 + 跟随活动提供商的只读生效状态），状态校验密钥锁保护 |
...
| **检索** | 三维度搜索：综合检索（分组展示◆事件原文/■知识节点/▲系统事件）/ 事件原文 / 知识节点；高级可折叠筛选面板（来源类型、图扩散跳数、最低置信度、结果数量）；翻页功能（服务端 offset 分页）；配置语义检索后自动纳入向量召回 |
